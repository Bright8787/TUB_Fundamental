...
public class ContentAwareImageResizing {
    ...
    public int[] leastContrastImageVPath() {
        // TODO
        WeightedDigraph picture =  makeVGraph();
        ShortestPathsTopological dijk = new ShortestPathsTopological(picture,sx*sy);
        Stack<Integer> parent = dijk.pathTo(sy*sx + 1);
        int[] contrastPath = new int[parent.size() - 2];
        //exclude first node
        parent.pop(); //remove last node sx*sy+1
        for(int i = 0; i < contrastPath.length; i++){
            contrastPath[i] = nodeToCoordinate(parent.pop()).x;
        }
        return contrastPath;
    }
...
//        demoMatrixImage("640px-Broadway_tower_edit.jpg");
        demoMatrixImage("640px-foto_2017_07_30_202914-cropped.jpg");
