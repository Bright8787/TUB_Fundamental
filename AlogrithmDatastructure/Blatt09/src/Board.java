...
public class Board {
    ...
    public int manhattan() {
        ...
        int heuristic = 0;
            ...
            for(    int i = 0; i < ny ; i ++){
                for(int j = 0 ; j < nx; j++){
                    if(field[i][j] != ny*i + j + 1 && field[i][j] != 0){
                        int realPosY = (field[i][j]-1) / ny;
                        int realPosX = (field[i][j]-1) % ny ;
                        heuristic += Math.abs(realPosX - j) + Math.abs(realPosY - i);
                    }
    ...
    public boolean equals(Object o) {
        ...
        Board board = (Board) o;
        return nx == board.nx && ny == board.ny && Objects.deepEquals(field, board.field) && Objects.equals(blank, board.blank);
    }
